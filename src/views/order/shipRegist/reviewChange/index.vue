<script lang="ts" setup>
import { computed, onMounted, reactive, ref, toRefs } from "vue"
import proTable from "@/components/proTable.vue"
import { getReviewChangeList, type ReviewChangeParams } from "@/api/order"
import { resetFields } from "@/utils"
import { columns, formatTableFun } from "./reviewChange"
import { useCodeListStore } from "@/stores/codeList"
const CodeListStore = useCodeListStore()
CodeListStore.getCodeList("customer")
CodeListStore.getCodeList("device")

const searchForm = ref<ReviewChangeParams>({
  po: "",
  old_po: "",
  sn: "",
  status: ""
})
const layout = {
  labelCol: { style: { width: "140px" } }
  // wrapperCol: { span: 20 }
}

const tableRef = ref()
const submit = () => {
  tableRef.value.doQuery()
}

const reset = () => {
  resetFields(searchForm.value)
  tableRef.value.doQuery()
}

const filterCode = (inputValue: string, option: any) => {
  return option.name.indexOf(inputValue) > -1
}
</script>
<template>
  <div class="page-main">
    <div class="page-search">
      <a-form ref="formRef" v-bind="layout" :model="searchForm" @finish="submit">
        <a-row :gutter="24">
          <a-col :span="8">
            <a-form-item label="变更销售合同编号">
              <a-select
                v-model:value="searchForm.po"
                show-search
                allowClear
                :options="CodeListStore.codeList['renewOrder']"
                :fieldNames="{ label: 'name', value: 'id' }"
                :filter-option="filterCode"
                @change="submit()"
              ></a-select>
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item label="出厂销售合同编号">
              <a-select
                v-model:value="searchForm.old_po"
                show-search
                allowClear
                :options="CodeListStore.codeList['renewOrder']"
                :fieldNames="{ label: 'name', value: 'id' }"
                :filter-option="filterCode"
                @change="submit()"
              ></a-select>
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item label="福达SN">
              <a-select
                v-model:value="searchForm.sn"
                show-search
                allowClear
                :options="CodeListStore.codeList['device']"
                :fieldNames="{ label: 'name', value: 'id' }"
                :filter-option="filterCode"
                @change="submit()"
              ></a-select>
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter="24">
          <a-col :span="8">
            <a-form-item label="审批状态">
              <a-select
                v-model:value="searchForm.status"
                show-search
                :options="CodeListStore.codeList['reviewState']"
                :fieldNames="{ label: 'name', value: 'id' }"
                :filter-option="filterCode"
              ></a-select>
            </a-form-item>
          </a-col>
          <a-col :span="16" style="text-align: right">
            <a-button type="primary" html-type="submit">查询</a-button>
            <a-button style="margin-left: 8px" @click="reset">重置</a-button>
          </a-col>
        </a-row>
      </a-form>
    </div>
    <div class="page-table">
      <pro-table
        ref="tableRef"
        :columns="columns"
        :reqParams="searchForm"
        :reqApi="getReviewChangeList"
        :formatTableFun="formatTableFun"
      >
        <template #bodyCell="{ column, text, record, index }">
          <template v-if="column.dataIndex === 'index'">
            <div>{{ index + (tableRef.pageData.pageNo - 1) * 10 + 1 }}</div>
          </template>
          <template v-if="column.dataIndex === 'remark'">
            <div style="color: red">
              {{ text }}
            </div>
          </template>
        </template>
      </pro-table>
    </div>
  </div>
</template>
<style lang="less" scoped>
.block {
  display: inline-block;
  content: "";
  width: 12px;
  height: 12px;
  margin-right: 3px;
}

.tag-1 {
  margin-left: 12px;
}

.tag-1::before {
  .block();
  background-color: #46a0eff7;
}

.tag-2::before {
  .block();
  background-color: #d6d0d0;
}
</style>
